...
Transform::Transform(float h11, float h12, float h13,
...
Transform::Transform(float h11, float h12, float h21,
...
Transform::Transform(const Matrix &mtx)
...
{
    Transform t(affine._m11, affine._m21, affine._dx,
...
{
    Transform transform(1, 0, 0, 0, 1, 0, dx, dy, 1, true);
    if (dx == 0 && dy == 0)
...
{
    Transform transform(sx, 0, 0, 0, sy, 0, 0, 0, 1, true);
    if (sx == 1. && sy == 1.)
        transform.m_type = TxNone;
    else
...
/*!
    ...
    \sa setMatrix()
*/
/*Transform &Transform::rotate(qreal a, Qt::Axis axis)
{
    if (a == 0)
        return *this;
#ifndef QT_NO_DEBUG
...
void Transform::setMatrix(float m11, float m12, float m13,
...
*/
const Matrix &Transform::toAffine() const
{
    return affine;
}
